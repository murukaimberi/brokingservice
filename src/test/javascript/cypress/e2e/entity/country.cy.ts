...
describe('Country e2e test', () => {
  ...
  const countrySample = { name: 'huge whoever', currency: 'inasmuch', currencyCode: 'TMT' };
  ...
  describe('new Country page', () => {
    ...
    it('should create an instance of Country', () => {
      cy.get(`[data-cy="name"]`).type('uh-huh vaccinate promptly');
      cy.get(`[data-cy="name"]`).should('have.value', 'uh-huh vaccinate promptly');

      cy.get(`[data-cy="currency"]`).type('though');
      cy.get(`[data-cy="currency"]`).should('have.value', 'though');

      cy.get(`[data-cy="currencyCode"]`).type('CVE');
      cy.get(`[data-cy="currencyCode"]`).should('have.value', 'CVE');

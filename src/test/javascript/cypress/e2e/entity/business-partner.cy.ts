...
describe('BusinessPartner e2e test', () => {
  ...
  const businessPartnerSample = {
    name: 'importune',
    description: 'Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci50eHQ=',
    representativeName: 'yum minus blah',
    email: 'Tamara49@yahoo.com',
    phoneNumber: 'urgently hm inwardly',
    agentType: 'INSURER',
  };
  ...
  describe('new BusinessPartner page', () => {
    ...
    it('should create an instance of BusinessPartner', () => {
      cy.get(`[data-cy="name"]`).type('highly minimise');
      cy.get(`[data-cy="name"]`).should('have.value', 'highly minimise');

      cy.get(`[data-cy="description"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="description"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="representativeName"]`).type('synthesize');
      cy.get(`[data-cy="representativeName"]`).should('have.value', 'synthesize');

      cy.get(`[data-cy="email"]`).type('Marques_Schamberger@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Marques_Schamberger@gmail.com');

      cy.get(`[data-cy="phoneNumber"]`).type('vice');
      cy.get(`[data-cy="phoneNumber"]`).should('have.value', 'vice');

      cy.get(`[data-cy="agentType"]`).select('INSURER');

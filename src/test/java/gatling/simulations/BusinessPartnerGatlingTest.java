...
public class BusinessPartnerGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all businessPartners").get("/api/business-partners").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new businessPartner")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"name\": \"SAMPLE_TEXT\"" +
                                ", \"description\": null" +
                                ", \"representativeName\": \"SAMPLE_TEXT\"" +
                                ", \"email\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_businessPartner_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created businessPartner").get("${new_businessPartner_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created businessPartner").delete("${new_businessPartner_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BusinessPartner entity").exec(scn);

...
public class ContractReportGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all contractReports").get("/api/contract-reports").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contractReport")
                        .post("/api/contract-reports")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"contractDocument\": null" + ", \"createdDate\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contractReport_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created contractReport").get("${new_contractReport_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created contractReport").delete("${new_contractReport_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ContractReport entity").exec(scn);

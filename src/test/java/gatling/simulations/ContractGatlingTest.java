...
public class ContractGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all contracts").get("/api/contracts").headers(headers_http_authenticated).check(status().is(200)))
                    ...
                    http("Create new contract")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"inception\": \"2020-01-01T00:00:00.000Z\"" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created contract").get("${new_contract_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created contract").delete("${new_contract_url}").headers(headers_http_authenticated))
                .pause(10)
        );

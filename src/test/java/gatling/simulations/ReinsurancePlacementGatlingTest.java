...
/**
 * Performance test for the ReinsurancePlacement entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.5.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class ReinsurancePlacementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all reinsurancePlacements")
                    .get("/api/reinsurance-placements")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reinsurancePlacement")
                        .post("/api/reinsurance-placements")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"riPercentageCommission\": 0" +
                                ", \"riPercentageShare\": 0" +
                                ", \"riTotalSumInsured\": 0" +
                                ", \"riLimitOfLiability\": 0" +
                                ", \"grossPremiumHundred\": 0" +
                                ", \"riPremium\": 0" +
                                ", \"riCommission\": 0" +
                                ", \"netDueFromInsurer\": 0" +
                                "}"
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reinsurancePlacement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created reinsurancePlacement").get("${new_reinsurancePlacement_url}").headers(headers_http_authenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created reinsurancePlacement")
                        .delete("${new_reinsurancePlacement_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ReinsurancePlacement entity").exec(scn);

 ...
 */
public class ContractHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all contractHistories").get("/api/contract-histories").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new contractHistory")
                        .post("/api/contract-histories")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"contractCreatedDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"contractActiveDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"contractInActiveDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"contractLastModifiedDate\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_contractHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created contractHistory").get("${new_contractHistory_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created contractHistory").delete("${new_contractHistory_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ContractHistory entity").exec(scn);

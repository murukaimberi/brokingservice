...
public class ClassOfBusinessGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all classOfBusinesses")
                    .get("/api/class-of-businesses")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new classOfBusiness")
                        .post("/api/class-of-businesses")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_classOfBusiness_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created classOfBusiness").get("${new_classOfBusiness_url}").headers(headers_http_authenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created classOfBusiness").delete("${new_classOfBusiness_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ClassOfBusiness entity").exec(scn);

...
type RestOf<T extends IContract | NewContract> = Omit<T, 'inception' | 'expiry'> & {
...
export class ContractService {
  ...
  protected convertDateFromClient<T extends IContract | NewContract | PartialUpdateContract>(contract: T): RestOf<T> {
    return {
      ...contract,
      inception: contract.inception?.toJSON() ?? null,
      expiry: contract.expiry?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restContract: RestContract): IContract {
    return {
      ...restContract,
      inception: restContract.inception ? dayjs(restContract.inception) : undefined,
      expiry: restContract.expiry ? dayjs(restContract.expiry) : undefined,
    };
  }
